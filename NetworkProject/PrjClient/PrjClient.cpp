...
#define LINE		  1001                   // 메시지 타입: 선 그리기
#define TRIANGLE	  1002                   // 메시지 타입: 삼각형 그리기
#define RECTANGLE	  1003					 // 메시지 타입: 사각형 그리기
#define CIRCLE		  1004				     // 메시지 타입: 원 또는 타원 그리기
...
static int			 g_type; //타입 저장
static int           g_color; //색깔 저장
static int           g_myType; //자신만의 타입 저장
static int           g_myColor; //자신만의 색깔 저장
                   ...
                   LPSTR lpCmdLine, int nCmdShow)
{
	...
	g_type = g_drawMsg.type;
	g_color = g_drawMsg.color;

	g_myType = g_drawMsg.type;
	g_myColor = g_drawMsg.color;
...
BOOL CALLBACK DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		hRectangle = GetDlgItem(hDlg, IDC_RECTANGLE);
		hCircle = GetDlgItem(hDlg, IDC_CIRCLE);
		...
		SendMessage(hLine, BM_SETCHECK, BST_CHECKED, 0);
		SendMessage(hTriangle, BM_SETCHECK, BST_UNCHECKED, 0);
		SendMessage(hRectangle, BM_SETCHECK, BST_UNCHECKED, 0);
		SendMessage(hCircle, BM_SETCHECK, BST_UNCHECKED, 0);
		...
		g_hDrawWnd = CreateWindow("MyWndClass", "그림 그릴 윈도우", WS_CHILD,
			450, 70, 425, 385, hDlg, (HMENU)NULL, g_hInst, NULL);
	...
	case WM_COMMAND:
		...
		case IDC_COLORRED:
			g_drawMsg.color = RGB(255, 0, 0);	
			g_color = RGB(255, 0, 0);
			g_myColor = RGB(255, 0, 0);
			return TRUE;

		case IDC_COLORGREEN:
			g_drawMsg.color = RGB(0, 255, 0);
			g_color = RGB(0, 255, 0);
			g_myColor = RGB(0, 255, 0);
			return TRUE;

		case IDC_COLORBLUE:
			g_drawMsg.color = RGB(0, 0, 255);
			g_color = RGB(0, 0, 255);
			g_myColor = RGB(0, 0, 255);
			return TRUE;
		...
		case IDC_TRIANGLE:
			g_drawMsg.type = TRIANGLE;
			g_type = TRIANGLE;
			g_myType = TRIANGLE;
			return TRUE;

		case IDC_RECTANGLE:
			g_drawMsg.type = RECTANGLE;
			g_type = RECTANGLE;
			g_myType = RECTANGLE;
			return TRUE;

		case IDC_CIRCLE:
			g_drawMsg.type = CIRCLE;
			g_type = CIRCLE;
			g_myType = CIRCLE;
			return TRUE;
...
DWORD WINAPI ReadThread(LPVOID arg)
{
	...
	while(1){
		...
		if(comm_msg.type == CHATTING){
			...
			g_type = draw_msg->type;
			g_color = draw_msg->color;
...
DWORD WINAPI WriteThread(LPVOID arg)
{
	...
	while(1){
		...
		int len = sizeof(int)+strlen(g_chatmsg.buf)+1;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
		if(bDrawing && g_bStart){
			...
			g_drawMsg.x1 = x1;
			g_drawMsg.y1 = y1;
			g_drawMsg.type = g_myType;
			g_drawMsg.color = g_myColor;
			int len = sizeof(g_drawMsg);
	...
	case WM_DRAWIT:
		hDC = GetDC(hWnd);
		hPen = CreatePen(PS_SOLID, 3, g_color);
		...
		if (g_type == LINE) {
			...
			int point1x = LOWORD(wParam);
			int point1y = HIWORD(lParam);
			int point2x = LOWORD(lParam);
			int point2y = HIWORD(lParam);
			int point3x = (LOWORD(wParam)+LOWORD(lParam))/2;
			int point3y = HIWORD(wParam);
			//화면에 그리기
			
			MoveToEx(hDC, point1x, point1y, NULL);
			LineTo(hDC, point2x, point2y);
			LineTo(hDC, point3x, point3y);
			LineTo(hDC, point1x, point1y);
			SelectObject(hDC, hOldPen);
			//메모리 비트맵에 그리기
			hOldPen = (HPEN)SelectObject(hDCMem, hPen);
			MoveToEx(hDCMem, point1x, point1y, NULL);
			LineTo(hDCMem, point2x, point2y);
			LineTo(hDCMem, point3x, point3y);
			LineTo(hDCMem, point1x, point1y);
			SelectObject(hDC, hOldPen);
			DeleteObject(hPen);
			DeleteObject(hOldPen);
		}
			...
			int a = pow(abs(LOWORD(lParam) - LOWORD(wParam))/2, 2);
			int b = pow(abs(HIWORD(lParam) - HIWORD(wParam))/2, 2);
			int centerx = (LOWORD(lParam) + LOWORD(wParam)) / 2;
			int centery = (HIWORD(lParam) + HIWORD(wParam)) / 2;
			int startx = LOWORD(wParam);
			int endx = LOWORD(lParam);
			float py;
			float my;
			MoveToEx(hDC, startx,  abs(HIWORD(lParam) + HIWORD(wParam))/ 2, NULL);
			MoveToEx(hDCMem, startx, abs(HIWORD(lParam) + HIWORD(wParam)) / 2, NULL);
			
			for (startx; startx <= endx; startx++) {
				py = sqrt(abs(b-(b*pow(startx-centerx,2))/a)) + centery;
				
				LineTo(hDC, startx, py);
				LineTo(hDCMem, startx, py);
			}
			startx = LOWORD(wParam);
			MoveToEx(hDC, startx, abs(HIWORD(lParam) + HIWORD(wParam)) / 2, NULL);
			for (startx; startx <= endx; startx++) {
				my = -sqrt(abs(b - (b * pow(startx - centerx, 2)) / a)) + centery;

				LineTo(hDC, startx, my);
				LineTo(hDCMem, startx, my);
			}
			SelectObject(hDC, hOldPen);
			DeleteObject(hPen);
			DeleteObject(hOldPen);
